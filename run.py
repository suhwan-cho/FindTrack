...
def test():
    ...
    clip, clip_preprocess = alpha_clip.load('ViT-L/14@336px', alpha_vision_ckpt_pth='weights/clip_l14_336_grit_20m_4xe.pth', device='cuda')
    ...
    for idx_, video in enumerate(video_list):
        ...
        for e in range(num_expressions):
            ...
            for ref_idx in range(ref_num):
                ...
                ref_mask, ref_score = evfsam.inference(imgs_sam[i].unsqueeze(0), imgs_beit[i].unsqueeze(0), words, resize_shape, original_size_list)
                ...
                image_features = clip.visual(imgs_clip[i].unsqueeze(0), alpha.unsqueeze(0))
            ...
            for i in range(best_i, video_len):
                if i == best_i:
                    mask_prob = processor.step(imgs_cutie[i], ref_masks[best_ref_idx].squeeze(0), objects=[1])
                else:
                    mask_prob = processor.step(imgs_cutie[i])
