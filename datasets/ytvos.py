...
class YTVOSDataset(Dataset):
    ...
    def __init__(self, args, img_folder: Path, ann_file: Path, tf, return_masks: bool,
    ...
    def bounding_box(img):
        ...
        return rmin, rmax, cmin, cmax # y1, y2, x1, x2 [y_min, y_max, x_min, x_max]
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            for j in range(self.num_frames):
                ...
                #img_sam = self._transforms.apply_image(img) ## SAM
                #img_sam = self.preprocess(torch.from_numpy(img_sam).permute(2, 0, 1).contiguous())
                img_sam,resize = sam_preprocess(img)
                img_beit = self.transforms_beit(img) ## BeIT
                ...
                if (mask > 0).any():
                    y1, y2, x1, x2 = self.bounding_box(mask) # y_min, y_max, x_min, x_max
                    box = torch.tensor([x1, y1, x2, y2]).to(torch.float) # x_min, y_min, x_max, y_max
                    valid.append(1)
