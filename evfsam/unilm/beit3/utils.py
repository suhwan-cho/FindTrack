...
def load_state_dict(model, state_dict, prefix='', ignore_missing="relative_position_index"):
    ...
    if len(missing_keys) > 0:
        print("Weights of {} not initialized from pretrained evfsam: {}".format(
            model.__class__.__name__, missing_keys))
    if len(unexpected_keys) > 0:
        print("Weights from pretrained evfsam not used in {}: {}".format(
            model.__class__.__name__, unexpected_keys))
    if len(ignore_missing_keys) > 0:
        print("Ignored weights of {} not initialized from pretrained evfsam: {}".format(
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if loss_scaler is not None:
        ...
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'evfsam': model_without_ddp.state_dict(),
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            ...
            model_without_ddp.load_state_dict(checkpoint['evfsam'])
